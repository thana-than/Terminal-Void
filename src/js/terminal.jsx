...
export const TerminalCommands = {
    ...
    DEV: {
        keys: ['dev'],
        get help() { return TerminalCommands.SUDO.help },
        get autoContexts() { return TerminalCommands.SUDO.autoContexts },
        accessKey: 'DEV',
        invoke: async function (params, context) {
            if (!params || params.length == 0)
                return "Requires command."
            ...
            return await TerminalCommands.SUDO.invokeAsSuperuser(context);
        }
    },
        ...
        invoke: async function (params, context) {
            ...
            if (result.node) //* Start by seeing if the param is a path to a directory
            {
                ...
                if (result.node.isFile) {
                    //* Run if file
                    return await TerminalCommands.RUN.invoke(params, context);
                }
                else {
                    //* Nav if folder
                    return TerminalCommands.CD.invoke(params, context);
                }
            }
    ...
    CD: {
        ...
        invoke: function (params, context) {
            ...
            if (newDir.success)
                return <>{TerminalCommands.LIST.invoke(null, context)}</>;
